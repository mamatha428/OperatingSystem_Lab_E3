...
int search(int key,int f[],int fo){
   for(int i=0;i<fo;i++)
      if(f[i]==key)
        return 1;
   return 0;
...
void printing(int k,int f[],int fo,int frames){
   cout<<"\n"<<k<<"\t\t";
   for(int i=0;i<frames;i++){
      if(i<fo){
        cout<<f[i]<<"\t\t";
...
int predict(int a[],int f[],int pages,int index,int fo){
   int res=-1,far=index;
   for(int i=0;i<fo;i++){ 
      int j;
      for(j=index;j<pages;j++){
          if(f[i]==a[j]){
             if(j>far){
                far=j;//silly mistake you did here
                res=i;
             }
            break;
          }
 ...
 void optimal_page(int a[],int f[],int pages,int frames){
     int frames_occupied=0;
     printOuterStructure(frames);
     int hits=0;
     for(int i=0;i<pages;i++){
        if(search(a[i],f,frames_occupied)){
          hits++;
          printing(a[i],f,frames_occupied,frames);
          continue;
        }
        if(frames_occupied<frames){
           f[frames_occupied]=a[i];
           frames_occupied++;
           printing(a[i],f,frames_occupied,frames);
        }
        else{
           int pos=predict(a,f,pages,i+1,frames_occupied);
           f[pos]=a[i];
           printing(a[i],f,frames_occupied,frames);
        }
     }
